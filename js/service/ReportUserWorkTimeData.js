reportApp.service('ReportUserWorkTimeData', function($window, $q, ReportData, ReportWorkTimeData, UserAppData, SharedData, _) {
    var UserData = function() {
        /*
            1. main 컨트롤러에서 period_duration을 넘겨 받는다.
            2. ReportUserWorkTimeData 서비스에서 duration을 확인하여 request 전에 work_from과 work_until값을 업데이트 한다.
            3. work_from과 work_until을 가져와서 request(유저 TOP10 데이터)를 총 5번을 수행한다.
            4. 수행된 결과 값(유저 TOP 10 데이터 50개)을 가지고 TOP10 사용자를 선출한다.
            5. request(선출된 TOP10 사용자에 대한 TOP3 앱 데이터)를 수행한다.
            6. 수행된 결과 값(선출된 TOP10 사용자에 대한 TOP3 앱 데이터)을 가지고 TOP3 앱을 선출한다.
         */
        this.q_userData = function(hostname, from, until, work_from, work_until, duration, isset) {
            ...
            var setWorkTime = function (days, interval) {
                ...
                _work_from = today.format('{yyyy}-{MM}-{dd}').raw+" 09:00:00";
                SharedData.setWorkFrom(_work_from);
                // until = today.format('{yyyy}-{MM}-{dd}').raw+" 18:00:00";
                _work_until = today.format('{yyyy}-{MM}-{dd}').raw+" 18:00:00";
                SharedData.setWorkUntil(_work_until);
                console.log("workfrom in setWorkTime : " + SharedData.getWorkFrom());
                console.log("workuntil in setWorkTime : " + SharedData.getWorkUntil());
            };

            var period_duration = SharedData.getPeriodDuration();
            console.log("ReportUserWorkTimeData: " + period_duration);
            ...
            if (isset) {
                ReportData.getUserActiveFlows(hostname).then(function (data) {
                    ...
                    // });
                    console.log("getUserActiveFlows: ", data);
                                    ...
                                    _arr_user_app_map.forEach(function (val, key) {
                                        ...
                                        _.each(val, function (e, i) {
                                            ...
                                            if (i === 0) {
                                                ...
                                                obj["top1_app_from"] = e.from.toLocaleString();
                                                obj["top1_app_until"] = e.until.toLocaleString();
                                                ...
                                                obj["top2_app_from"] = e.from.toLocaleString();
                                                obj["top2_app_until"] = e.until.toLocaleString();
                                                ...
                                                obj["top3_app_from"] = e.from.toLocaleString();
                                                obj["top3_app_until"] = e.until.toLocaleString();
